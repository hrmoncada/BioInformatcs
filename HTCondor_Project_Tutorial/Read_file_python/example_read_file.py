...
with open('data_history_1_1_12.txt', 'r') as infile:
...
RTime = Run_Time_min*60 + Run_Time_sec
print "Count   ID    End    Start    TIME  RunTime   Rtime"
for i in range(0, count-1): # full file
  ...
  print "   %d  %4.3f   %d    %d     %d    %d    %d" %  (i, ID[i], min_End[i]*60.0 + sec_End[i], min_Start[i]*60.0 + sec_Start[i], min_End[i]*60.0 + sec_End[i]- min_Start[i]*60.0 + sec_Start[i], Run_Time_min[i]*60 + Run_Time_sec[i], RTime[i])
  if i == 10:
     raw_input("Press Enter to continue...")

print "Maximun = %d, Minimun = %d"% (max(RTime), min(RTime))
#print len(min_Start)
#print "\n" #"Sum = %d"% sum(vec)"""
#print len(sec_Start)
